    ...
    def __init__(self):
        self.epd = epd.EPD(
            spi=machine.SPI(
                ...
                sck=machine.Pin(config.Pins.SCK),
                mosi=machine.Pin(config.Pins.MOSI),
            ),
            m1_cs=machine.Pin(config.Pins.M1_CS, machine.Pin.OUT),
            s1_cs=machine.Pin(config.Pins.S1_CS, machine.Pin.OUT),
            m2_cs=machine.Pin(config.Pins.M2_CS, machine.Pin.OUT),
            s2_cs=machine.Pin(config.Pins.S2_CS, machine.Pin.OUT),
            m1s1_dc=machine.Pin(config.Pins.M1S1_DC, machine.Pin.OUT),
            m2s2_dc=machine.Pin(config.Pins.M2S2_DC, machine.Pin.OUT),
            m1s1_rst=machine.Pin(config.Pins.M1S1_RST, machine.Pin.OUT),
            m2s2_rst=machine.Pin(config.Pins.M2S2_RST, machine.Pin.OUT),
            m1_busy=machine.Pin(config.Pins.M1_BUSY, machine.Pin.IN),
            s1_busy=machine.Pin(config.Pins.S1_BUSY, machine.Pin.IN),
            m2_busy=machine.Pin(config.Pins.M2_BUSY, machine.Pin.IN),
            s2_busy=machine.Pin(config.Pins.S2_BUSY, machine.Pin.IN),
        )
